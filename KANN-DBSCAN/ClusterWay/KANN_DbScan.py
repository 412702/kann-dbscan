...
def kannDbscan(dataSet, dimesionName):
    """
    ...
    #showRelationLineChart(EpsCandidate, "Eps of "+dimesionName)    #画出K与Eps候选集的关系
    end = time.perf_counter()
    ...
    #showRelationLineChart(MinptsCandidate, "MinPts of "+dimesionName)    #画出K与MinPts候选集的关系
    end = time.perf_counter()
    print('finish Minpts Candidates in %s s of %s' % (str(end - start),dimesionName))
    
    Density = returnDensity(EpsCandidate, MinptsCandidate)
    #showRelationLineChart(Density, "Density Result of "+dimesionName)    #画出K与Density的关系
    end = time.perf_counter()
    ...
    #showRelationLineChart(ClusterNumberList, "Cluster Result Number of "+dimesionName)   #画出K与最终聚类结果聚类簇数的关系
    end = time.perf_counter()
